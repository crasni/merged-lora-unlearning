...
def train_lora(config: Config, role: str) -> Path:
    ...
    details = (
        f"facts={len(facts)} examples={len(facts) * 2} epochs={config.acquisition.epochs} "
        f"batch={config.acquisition.batch_size} rank={config.acquisition.lora_rank}"
    )
    with stage(f"train:{role}", details):
        ...
        info(f"Saving {role} adapter and tokenizer")
    ...
    info(f"Saved {role} adapter: {output_dir}")
    return output_dir
